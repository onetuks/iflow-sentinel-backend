...
class SapODataClientTest {
    ...
    void getCollectionFetchesTokenThenCallsODataWithBearerAuth() {
        mockServer.expect(requestTo(TOKEN_URL))
                ...
                .andRespond(withSuccess("{\"access_token\":\"tok-abc\",\"expires_in\":3600,\"token_type\":\"bearer\"}", MediaType.APPLICATION_JSON));
        mockServer.expect(requestTo(ODATA_URL + "/IntegrationPackages"))
                .andExpect(method(HttpMethod.GET))
                .andExpect(header(HttpHeaders.AUTHORIZATION, "Bearer tok-abc"))
                .andRespond(withSuccess("{\"d\":{\"results\":[{\"Id\":\"PKG1\",\"Name\":\"Package One\"}]}}", MediaType.APPLICATION_JSON));

        List<SapPackageDto> packages = odataClient.getCollection(
                tenant, "/IntegrationPackages", new ParameterizedTypeReference<ODataCollectionResponse<SapPackageDto>>() {
                });
    ...
    void getBinaryReturnsRawBytes() {
        mockServer.expect(requestTo(TOKEN_URL))
                .andExpect(method(HttpMethod.POST))
                .andRespond(withSuccess("{\"access_token\":\"tok-abc\",\"expires_in\":3600,\"token_type\":\"bearer\"}", MediaType.APPLICATION_JSON));
        mockServer.expect(requestTo(ODATA_URL + "/IntegrationDesigntimeArtifacts(Id='ART1',Version='1.0.0')/$value"))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess(new byte[]{1, 2, 3, 4}, MediaType.APPLICATION_OCTET_STREAM));

        byte[] result = odataClient.getBinary(tenant, "/IntegrationDesigntimeArtifacts(Id='ART1',Version='1.0.0')/$value");
