...
class RequiredLoggingEvaluatorTest {
    ...
    void matchingRequiredLevelProducesNoFindings() {
        Rule rule = RuleEngineTestSupport.rule(RuleType.REQUIRED_LOGGING, Map.of(), Map.of("required", "All events"), "무시");
    ...
    void mismatchingRequiredLevelProducesOneFinding() {
        Rule rule = RuleEngineTestSupport.rule(RuleType.REQUIRED_LOGGING, Map.of(), Map.of("required", "Trace"), "로그 수준이 부족합니다.");
