...
final class RuleEngineTestSupport {
    ...
    static ParsedModel realFixtureModel() {
        try (InputStream in = RuleEngineTestSupport.class.getResourceAsStream("/parser/GMES_GQMS_EA_PQCRESULT_01.zip")) {
            if (in == null) {
    ...
    /** channels[]만 채우고 나머지는 빈 값으로 채운 최소 ParsedModel. 실제 픽스처가 커버하지 못하는 경우(ProcessDirect 등) 전용. */
    static ParsedModel modelWithChannels(List<Channel> channels) {
        ...
        IflowModel iflow = new IflowModel(config, List.of(), channels, List.of(), List.of(), List.of(), List.of(), List.of(), List.of());
        ArtifactInfo artifactInfo = new ArtifactInfo("Test", "Test", "1.0.0", "IntegrationFlow", "iflmap", "", 0L, List.of());
