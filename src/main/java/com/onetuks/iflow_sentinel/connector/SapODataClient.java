...
/**
 * SAP IS OData API 인증 호출 공용 헬퍼. {@code tenant.odataUrl()}은 "/api/v1"까지 포함한 베이스 URL로 가정한다
...
public class SapODataClient {
    ...
    public <T> List<T> getCollection(Tenant tenant, String relativePath, ParameterizedTypeReference<ODataCollectionResponse<T>> typeRef) {
        ...
        try {
            ...
            throw new ConnectorException("OData 호출 실패: HTTP " + e.getStatusCode().value() + " " + relativePath, e.getStatusCode().value(), e);
    ...
    public byte[] getBinary(Tenant tenant, String relativePath) {
        ...
        try {
            ...
            throw new ConnectorException("아티팩트 다운로드 실패: HTTP " + e.getStatusCode().value() + " " + relativePath, e.getStatusCode().value(), e);
        } catch (ResourceAccessException e) {
